...
class HomeViewModel @Inject constructor(
    ...
    fun getFcmToken() {
        viewModelScope.launch {
            try {
                val token = fcmTokenUseCase.getFcmToken()
    ...
    fun addRecord(record: Record) {
        viewModelScope.launch {
            try {
                val result = saveDrinkUseCase.saveDrink(record)
    ...
    fun getTodayRecords() {
        viewModelScope.launch {
            try {
                val result = todayRecordsUseCase.getTodayRecords()
    ...
    fun deleteRecord(id: String) {
        viewModelScope.launch {
            try {
                val result = deleteRecordUseCase.deleteRecord(id)
    ...
    fun getAdvices() {
        viewModelScope.launch {
            try {
                val result = getAdvicesUseCase.getAdvices()
